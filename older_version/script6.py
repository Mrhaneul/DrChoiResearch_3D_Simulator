...
######################################################################################################################################################
 # Draw the light ray that is perpendicular to the line connecting the light source and the gravity source
    ...
    pygame.draw.line(window, (255, 255, 255), light_source, light_ray_end_position, 2)

    # Draw another light ray that is reflected over the line connecting the light source and the gravity source
    reflected_angle = 2 * angle - perpendicular_angle
    reflected_light_ray_end_x = GRAVITY_SOURCE_X + math.cos(math.radians(reflected_angle)) * DISTANCE_BETWEEN_LIGHT_RAY_AND_GRAVITY_SOURCE
    reflected_light_ray_end_y = GRAVITY_SOURCE_Y + math.sin(math.radians(reflected_angle)) * DISTANCE_BETWEEN_LIGHT_RAY_AND_GRAVITY_SOURCE
    reflected_light_ray_end_position = pygame.Vector2(reflected_light_ray_end_x, reflected_light_ray_end_y)
    pygame.draw.line(window, (255, 255, 255), light_source, reflected_light_ray_end_position, 2)

    if(angle > 0):
        print("Angle is positive")
        angle_360 = abs(angle + perpendicular_angle)
        ray_finish_x_360 = GRAVITY_SOURCE_X - math.cos(math.radians(angle_360)) * light_source.distance_to(reflected_light_ray_end_position)
        ray_finish_y_360 = GRAVITY_SOURCE_Y - math.sin(math.radians(angle_360)) * light_source.distance_to(reflected_light_ray_end_position)
        ray_finish_position_360 = pygame.Vector2(ray_finish_x_360, ray_finish_y_360)
        pygame.draw.line(window, (255, 255, 255), reflected_light_ray_end_position, ray_finish_position_360, 2)

        angle_180 = abs(angle + perpendicular_angle) + 180
        ray_finish_x = GRAVITY_SOURCE_X + math.cos(math.radians(angle_180)) * light_source.distance_to(light_ray_end_position)
        ray_finish_y = GRAVITY_SOURCE_Y + math.sin(math.radians(angle_180)) * light_source.distance_to(light_ray_end_position)
        ray_finish_position = pygame.Vector2(ray_finish_x, ray_finish_y)
        pygame.draw.line(window, (255, 255, 255), light_ray_end_position, ray_finish_position, 2)
    else:
        print("Angle is negative")
        angle_180 = abs(angle - 2 * perpendicular_angle)
        ray_finish_x = GRAVITY_SOURCE_X + math.cos(math.radians(angle_180)) * light_source.distance_to(light_ray_end_position)
        ray_finish_y = GRAVITY_SOURCE_Y + math.sin(math.radians(angle_180)) * light_source.distance_to(light_ray_end_position)
        ray_finish_position = pygame.Vector2(ray_finish_x, ray_finish_y)
        pygame.draw.line(window, (255, 255, 255), light_ray_end_position, ray_finish_position, 2)

        angle_360 = abs(angle - 2 * perpendicular_angle)
        ray_finish_x_360 = GRAVITY_SOURCE_X - math.cos(math.radians(angle_360)) * light_source.distance_to(reflected_light_ray_end_position)
        ray_finish_y_360 = GRAVITY_SOURCE_Y - math.sin(math.radians(angle_360)) * light_source.distance_to(reflected_light_ray_end_position)
        ray_finish_position_360 = pygame.Vector2(ray_finish_x_360, ray_finish_y_360)
        pygame.draw.line(window, (255, 255, 255), reflected_light_ray_end_position, ray_finish_position_360, 2)
